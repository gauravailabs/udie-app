...
// Simulate streaming with typewriter effect on the title
...
function StreamingInsightCard({ insight, onExpand }) {
  ...
  const { displayed: signalText, done: signalDone } = useTypewriter(
    titleDone ? insight?.signal : '', 12, titleDone
  );

  useEffect(() => {
    if (titleDone) setPhase(1);
  }, [titleDone]);
  useEffect(() => {
    if (signalDone && signalText) setPhase(2);
  }, [signalDone, signalText]);
  useEffect(() => {
    if (phase === 2) { const t = setTimeout(() => setPhase(3), 400); return () => clearTimeout(t); }
    if (phase === 3) { const t = setTimeout(() => setPhase(4), 800); return () => clearTimeout(t); }
  }, [phase]);
  ...
  return (
    <div className={isStreaming ? 'stream-card' : 'insight-card card-pressable'}
      ...
      <div style={{ fontFamily:'var(--font-display)', fontSize:17, fontWeight:700, letterSpacing:'-0.03em', lineHeight:1.3, color:'var(--text)', marginBottom:10, minHeight:24 }}>
        {titleText}
      ...
      {phase >= 1 && signalText && (
        <div style={{ fontSize:13, color:'var(--text2)', lineHeight:1.6, marginBottom:10 }}>
          {signalText}
          {phase === 1 && <span className="stream-cursor" />}
        </div>
      )}
      ...
      {phase >= 3 && insight?.actions?.length > 0 && (
        <div style={{ marginTop:10, animation:'cardIn .3s ease both' }}>
          ...
          {insight.actions.slice(0, 2).map((a, i) => (
            <div key={i} style={{ display:'flex', gap:8, fontSize:12, color:'var(--text2)', marginBottom:4, lineHeight:1.4 }}>
              <span style={{ color:'var(--blue2)', flexShrink:0 }}>•</span>
              {a.action}
            </div>
          ))}
        </div>
...
export default function IntelligenceScreen({ navigate }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const withMode = { ...insight, mode: activeMode, _streaming: true };
      setResults(prev => [withMode, ...prev]);
      saveInsightLocally(withMode);
      if (user?.id) saveInsightToDB(withMode, user.id).catch(() => {});
      setTimeout(() => {
        contentRef.current?.scrollTo({ top: 0, behavior: 'smooth' });
      }, 100);
    } catch (e) {
      setError(e.message || 'Intelligence generation failed. Check your API key (see DEPLOY.md) and make sure dev-server.js is running.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const clearAll = () => { if (confirm('Clear all results?')) setResults([]); };
    ...
    <div className="screen">
      ...
      <div style={{ padding:'16px 14px 0', flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:4 }}>
          <div style={{ fontFamily:'var(--font-display)', fontSize:22, fontWeight:800, letterSpacing:'-.04em' }}>
            Intelligence Query
          </div>
          {results.length > 0 && (
            <button onClick={clearAll} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--text3)', display:'flex', alignItems:'center', gap:4, fontSize:12 }}>
              <Trash2 size={13}/> Clear
            </button>
          )}
        </div>
        <div style={{ fontSize:13, color:'var(--text2)', marginBottom:12 }}>
          Ask anything. Get decision-ready insights.
        </div>
        ...
        <div className="modes-scroll" style={{ margin:'0 -14px', padding:'0 14px 2px' }}>
          {Object.values(MODES).map(m => (
            <button key={m.id}
              ...
              style={activeMode===m.id ? { borderColor:m.color, background:`${m.color}15`, boxShadow:`0 0 0 1px ${m.color}25` } : {}}
        ...
        <div style={{ fontSize:11, color:'var(--text3)', marginTop:8, marginBottom:12, minHeight:14, letterSpacing:'.01em' }}>
          {MODES[activeMode]?.desc}
        </div>
      </div>

      {/* Scrollable content */}
      <div ref={contentRef} className="content" style={{ paddingTop:0, paddingBottom:0 }}>
        ...
        {error && (
          <div className="alert alert-error" style={{ marginBottom:12 }}>
            ...
            <div style={{ marginTop:6, fontSize:12, opacity:.8 }}>
              Make sure you've run <code style={{ background:'rgba(255,77,106,.15)', padding:'1px 5px', borderRadius:4 }}>node dev-server.js</code> in a separate terminal.
            </div>
          </div>
        )}
          ...
          <div className="typing-indicator">
            ...
            <div style={{ flex:1 }}>
              <div style={{ fontSize:14, fontWeight:600, color:'var(--text)', marginBottom:3 }}>
                {MODES[activeMode]?.label} Intelligence Engine working…
              </div>
              <div style={{ fontSize:12, color:'var(--text2)' }}>
                Detecting signals · Contextualizing · Formulating actions
              </div>
            </div>
        ...
        {/* Results with streaming animation */}
      ...
      <div style={{ padding:'10px 14px', paddingBottom:`calc(10px + env(safe-area-inset-bottom,0px))`, background:'rgba(3,8,15,.96)', backdropFilter:'blur(24px)', borderTop:'1px solid var(--border)', flexShrink:0 }}>
        <div className="query-box">
          <textarea ref={textRef} className="query-input"
            ...
            onKeyDown={handleKey}
          />
          <button className="query-submit" onClick={submit} disabled={!query.trim() || loading}>
