...
// ─── Share helpers ────────────────────────────────────────────────────────────
function buildShareText(insight) {
  const mode  = MODES[insight?.mode];
  const emoji = mode?.icon || '🧠';
  const urgencyEmoji = insight?.urgency === 'high' ? '🔴' : insight?.urgency === 'medium' ? '🟡' : '🟢';

  return `${emoji} *${insight?.title}*

${urgencyEmoji} Urgency: ${insight?.urgency?.toUpperCase()} · ${insight?.urgency_timeframe || ''}
...
⚡ *Top Actions*
${(insight?.actions || []).slice(0,2).map((a,i) => `${i+1}. ${a.action} (${a.owner}, ${a.timeline})`).join('\n')}
...
function ShareBar({ insight }) {
  ...
  const shareText = buildShareText(insight);
